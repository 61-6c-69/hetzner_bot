...
class ServerRepository(BaseRepository[Server]):
    ...
    async def get_all_running_servers(
            self
    ) -> List[Server]:
        """Get user's servers with optional sync"""
        result = await self.db.execute(
    ...
    async def reset(self, server_id: int, user_id: Optional[int] = None) -> Server:
        """Power on server"""
        server = await self.get_server_with_sync(server_id, user_id)

        try:
            await hetzner.reset(server.hetzner_id)
            server.status = 'resetting'
            return server
        except Exception as e:
            raise HTTPException(
                status_code=500,
                detail=f"Error reset on server: {str(e)}"
            )
    ...
    async def change_action(self, server_id: int, user_id: Optional[int] = None) -> Server:
        """Power off server"""
        server = await self.get_server_with_sync(server_id, user_id)
    ...
    async def get_user_active_servers(self, user_id):
        servers = await self.get_user_servers(user_id)
        return [server for server in servers if server.status == "running"]
